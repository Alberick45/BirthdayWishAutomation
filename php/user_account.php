                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr class="table-secondary"> 
                            <th id="<?php echo $messageID; ?>" scope="row"> 
                                <!-- Trigger modal with dynamic data -->
                                <a href="#" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#update-message-modal-<?php echo $messageID; ?>">Edit</a> |
                                <a class="btn btn-outline-danger" href="messages.php?action=delete&mid=<?php echo $messageID; ?>" role="button">Delete</a>
                            </th> 
...
$sql_msg_options = "SELECT m_id, m_body FROM messages WHERE m_type='sample' OR m_ruid = '$userid'";
...
if ($msg_result->num_rows > 0) {
    while ($msg_row = $msg_result->fetch_assoc()) {
        ...
        $messagebody = htmlspecialchars($msg_row["m_body"]);
